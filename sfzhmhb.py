...
'''
完成了身份证号码的列表ls_sfz的构造，该列表完成了身份证号码的去重处理，包含所有身份证号码，并且每个身份证号码只出现一次。
这是一个思维难点。
'''
...
'''
将中间结果按身份证号码进行数据分割分别输出excel，暂时注释掉这个功能
'''
#for sfzhm in ls_sfz:
...
def proc_row(tuple_indivi,df_sfz2):
    ...
    for row in range(tuple_indivi[0]):    #遍历每一行
        ...
        if type(cell_value)==str:
            ...
            ls_hb=list(dict_hb)   #将转置后的df的每一行（即每个字段）构造的字典转换成列表
            print(row)
            str_hb=",".join(ls_hb)   #将列表转换成字符串
            col_end=tuple_indivi[1]  #用于在df最后一列后面构造新的列
...
按身份证号码分离每个人的数据
'''
i=1  #创建一个计数器，用于记录处理到第几个人了，也就是下面这个身份证号码循环到第几个人了
for sfzhm in ls_sfz:   #遍历每一个身份证号码
#sfzhm="35262719810104301X"
    tf=df_sfz["客户身份证号码"]==sfzhm   #构造用于数据筛选的series,将符合条件的行全部筛选出来了，并且标记为True,df_sfz["客户身份证号码"]取出的值包含重复值
    df_sfz2=df_sfz.loc[tf==True,:]  #将相同身份证号码的所有数据全部取出来
    df_sfz2=df_sfz2.fillna({"额度合同号": "空值"})    #将没有值的单元格使用“空值”填充
    df_sfz2=df_sfz2.T #将筛选出来的每个人的数据的dataframe进行行列转置，便于数据合并并写入新加列“合并”
    ...
    df_sfz2["合并"]="NaN"  #添加一个新的列用于存放每行合并后的数据，先使用NaN填充新的列。
    ...
    #df_sfz2=df_sfz2.T
    df_sfz_mb=df_sfz_mb.T
    df_sfz_mb.iloc[:,i]=df_sfz2.loc[:,"合并"]   #将每次调用函数后生成的df_sfz2中“合并”一列的数据填写到df_sfz_mb中。
    i=i+1 #在这里打一个断点结合变量i，可以用于实现对断点设置精细条件
